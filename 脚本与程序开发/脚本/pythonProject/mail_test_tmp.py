...
class send_mail:
    ...
    def send_mail(self, mail_info):
        for mail_name, mail_sql, mail_user, mail_cc, sub_title, _id, is_backup, backup_table_name in mail_info:
            ...
            mail_name = u'%s' % (re.sub(r'$', str(self.yday), str(mail_name)))
            if mail_sql:
...
def log_into_db(mail_title, context, *, db_conf=None, event_type=None):
    ...
    :param context: 日志内容
    :param db_conf: 数据库配置
    :param event_type: 事件类型，1:获取到了sql,开始邮件事件，开始执行sql,2:sql执行成功,生成execl,3:生成execl成功，发送邮件,4:邮件发送成功，事件结束,11:获取sql列表失败,12:sql执行失败,13:execl生成失败,14:邮件发送失败
    """
...
def wechat_alert(alert_type, *, mail_title=None, extra_content=''):
    ...
    content = ''
    Subject = '邮件发送失败\n' if int(alert_type) > 10 else '邮件发送成功\n'
    if mail_title:
        content += f'邮件标题: {mail_title}\n'
    content += '告警内容:'
    if alert_type == 4:
        content = content + f'今日邮件发送事件结束 \n'
    elif alert_type == 11:
        ...
        with pymysql.connect(**db_conf) as conn:
            for backup_table_name_one in backup_table_name_list:
                ...
                for one_column_name in mail_description:
                    if one_column_name[0] != 'create_time':
...
if __name__ == '__main__':
    ...
    log_into_db(f'邮件事件结束', '', db_conf=mail_config, event_type=20)
    if time.strftime("%H:%M") == '01:00':
        wechat_alert(4)
