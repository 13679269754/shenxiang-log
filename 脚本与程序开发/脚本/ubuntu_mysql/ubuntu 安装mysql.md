```bash
bash mysql_install.sh  --package_dir /root/soft --package_file mysql-8.0.23-el7-x86_64.tar.gz --port 3306 --data_path /usr/local/data/mysql_data/ --base_path /usr/local/data/mysql
```
...
function create_mysql_server() {
    ...
    #install mysql
    apt install -y make gcc g++ cmake libbison-dev libncurses5-dev bison pstack openssl libaio1t64 autoconf
...
function create_mysql_instance(){
    ...
    chown -R mysql:mysql $data_dir/db$dbport
...
function mysql_startup(){
    ...
    chown -R mysql:mysql /home/mysql/$dbport-login.sh

    chmod u+x /home/mysql/$dbport-start.sh
    chown -R mysql:mysql /home/mysql/$dbport-start.sh
